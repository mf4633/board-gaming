...
`,
  },
  {
    ...
    body: `
    <p id="eclipse-countdown" style="background:#141c30;border:1px solid #5a5080;border-radius:4px;padding:14px 18px;color:#ffd040;letter-spacing:1px;"></p>
    <script>
    (function(){
      var el = document.getElementById('eclipse-countdown');
      var d = Math.ceil((Date.UTC(2026,7,12) - Date.now()) / 86400000);
      if (d > 1) el.textContent = '🌒 The August 12, 2026 total eclipse is ' + d + ' days away.';
      else if (d >= 0) el.textContent = '🌒 The August 12, 2026 total eclipse is ' + (d === 0 ? 'TODAY' : 'tomorrow') + '.';
      else {
        var d27 = Math.ceil((Date.UTC(2027,7,2) - Date.now()) / 86400000);
        el.textContent = d27 >= 0 ? '🌒 Next up: the August 2, 2027 total eclipse — ' + d27 + ' days away.' : '';
