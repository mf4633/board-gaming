...
// Derived, not typed: this number appears on the card every share of the
// homepage renders, and it had already drifted one behind games.json.
const GAME_COUNT = JSON.parse(
  fs.readFileSync(path.join(ROOT, 'games.json'), 'utf8')
).games.length;

const CARDS = [
  { file: 'home', title: 'BOARD GAMING HUB', sub: `${GAME_COUNT} free browser games — no signup, no install`, accent: '#f0c060', bg: '#0c1016' },
